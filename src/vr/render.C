...
{
  // empty default constructor to avoid compilation error;
}
...
void volumeRender::out_to_image(char* filename)
{
  ...
  for (int i=0; i<udim; i++)     // init each pixel as black first to avoid bug when updating the image in flex vrlib
    for (int j=0; j<vdim; j++) {
      int idx = i*udim+j; 
      red[idx]   = (unsigned char) 0; 
      green[idx] = (unsigned char) 0; 
      blue[idx]  = (unsigned char) 0; 
    }
...
void volumeRender::update_transform()
{
  ...
  s.rotate( (short) (xangle*10 + 0.5), 'x'); // *10 + 0.5 for uknown outdated numerical issues
...
//
  ...
  // glm version view transformation. NOT WORKING
  ...
  // vrlib original view tranformation

  // s.scale(-1.0,1.0,1.0);  // flip over the image, and reverse Z
  // s.translate(1.0,1.0,1.0);

  // zscale = 1.0/(sc * STEPSIZE);
  // // move from [-1, 1] to screen space
  // s.scale((REAL)udim/2.0,(REAL)vdim/2.0,zscale);
  
  vp.scale(-1.0,1.0,1.0);  // flip over the image, and reverse Z
  vp.translate(1.0,1.0,1.0);

  zscale = 1.0/(sc * STEPSIZE);
  // move from [-1, 1] to screen space
  vp.scale((REAL)udim/2.0,(REAL)vdim/2.0,zscale);
  vp.getmatrix(world_to_screen);

  s.multmatrix(world_to_screen);
  s.getmatrix(data_to_screen);
  vrlib_invert_matrix(data_to_screen,screen_to_data);
}

// void volumeRender::update_vp(){
//   Trans_Stack s;
//   s.loadmatrix(data_to_world);
//   // vrlib original view tranformation
//   s.scale(-1.0,1.0,1.0);  // flip over the image, and reverse Z
//   s.translate(1.0,1.0,1.0);

//   int dmax = MAX(vxdim,MAX(vydim,vzdim));
//   REAL sc = 0.8 * 2.0/(REAL)dmax;
//   REAL zscale = 1.0/(sc * STEPSIZE);
//   // move from [-1, 1] to screen space
//   s.scale((REAL)udim/2.0,(REAL)vdim/2.0,zscale);
...
void volumeRender::update_rotation(std::vector<short> degrees, std::vector<char> axes) {
  assert (degrees.size() == axes.size());
  ...
  for (int i = degrees.size()-1; i >= 0; --i) {
    ...
    s.rotate((short) deg*10 + 0.5, axes[i]);
    std::cout << "Rotated " << i+1 << "\n";
  }
  s.getmatrix(data_to_world);
  vrlib_invert_matrix(data_to_world, world_to_data);
...
void volumeRender::update_viewing() 
{
  ...
  std::cout << "eye_W: " <<eye_W[0] << eye_W[1] << eye_W[2] << eye_W[3] << "\n";
...
int volumeRender::readCmapFile(char *filename)
{
    ...
    //std::cout << "First: " << cname << ' ' << type << ' ' << numRegions << ' ' <<
    // lookupSize << ' ' << norm_min << ' ' << norm_max << ' ' << 
    // vol_min << ' ' << vol_max << "\n\n";

    float range = vol_max - vol_min;
    curMin = vol_min + norm_min * range;
    curMax = vol_min + norm_max * range;
    //std::cout << "Second: " << range << ' ' << curMin << ' ' << curMax << "\n\n";
    ...
    //std::cout << "Third: " << cname << ' ' << type << ' ' << numCP << "\n\n";
